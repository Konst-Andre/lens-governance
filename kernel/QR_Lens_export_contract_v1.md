> живе доки: назавжди (вічне, wsd 1.8) — контракт експорту QR Lens

# QR Lens — Export Contract (template-build recipe)

**Версія:** v1 · **Виведено з:** `QR_Lens___html_export.xlsm` (VBA `ExportModule`/`Module1`) + `QR_Lens_preview_batch58_2.html`, 09.07.2026.
...
- `city` = коротка назва міста, `a` = адреса без міста (`ExtractCity`/`ShortAddress`, regex на «обл./м./смт./с.»).
...
| `ExtractCity`/`ShortAddress`/`RemovePSRPrefix`/`EscapeJSON` | парс/очистка полів |
