...
### (2-б) 🔴 Скло вмирає ТРЬОМА способами — контент позаду це лише перший (21.08.2026, EquipLens S5)

Частина (2) вище лікує **причину №1**: позаду нічого не намальовано. Є ще дві, і вони
підступніші, бо код виглядає правильним, а `backdrop-filter` у DevTools присутній.

**Спільний корінь усіх трьох — поняття `backdrop-root`.** Фільтр бере на вхід не «весь
екран позаду», а зображення **найближчого backdrop-root предка**. Зробив когось із предків
backdrop-root'ом — і фільтр чесно розмиває те, що між цим предком і елементом, тобто
здебільшого порожнечу. Ефект **не падає з помилкою**: він лишається на місці й тихо бреше.

**Тригери backdrop-root** (Filter Effects 2, §3.2 Backdrop Root Triggers):
корінь документа · `filter`≠none · **`opacity`<1** · **`mask` / `mask-image` / `mask-border` /
`clip-path`≠none** · `backdrop-filter`≠none · `mix-blend-mode`≠normal.
Специфікація пояснює причину прямо: маски фактично змінюють прозорість частин елемента,
тож поводяться подібно до `opacity`.

**Причина №2 · `opacity` замість alpha.**

```css
.glass{ opacity:.8; backdrop-filter:blur(20px); }   /* 🔴 скло мертве */
.glass{ background:rgba(255,255,255,.4); backdrop-filter:blur(20px); }  /* ✅ */
```

> **Закон: прозорість скла — це альфа-канал `background-color`, НІКОЛИ не `opacity`.**
> `opacity<1` робить елемент backdrop-root, і фільтру не лишається що розмивати.
> Альфа фону backdrop-root **не створює**.
>
> **Наслідок для стендів:** важіль «прозорість скла» мусить крутити **alpha фону**.
> Важіль `opacity` на скляному елементі — інстанс «мертвий за версткою»
> (`Lens_stagebench_manifest.md` §8.7-б): він фізично не має предмета дії, бо вбиває
> той самий ефект, який нібито налаштовує.

**Причина №3 · маска на батьку-обгортці.**

Класичний рефакторинг «винесу маску вирізу на обгортку, щоб не дублювати» вбиває скло
всередині — обгортка стає backdrop-root. **Маска мусить лежати на тому ж елементі,
що й `backdrop-filter`.** Це працює за специфікацією й дає найнадійніший крос-браузерний
результат — розгорнуто в `Lens_glass_FINDINGS.md` §9a/§9b, разом із чотирма кандидатами
реалізації вирізу.

**Плюс сумісність, не пов'язана з backdrop-root:** безпрефіксний `backdrop-filter`
з'явився лише в **Safari 18** (червень 2024). Safari 9–17 і старші iOS малюють блюр
**тільки** з `-webkit-`. **Обидва рядки лишаються завжди** — це не legacy-сміття.

**Детектор.** Скло на вигляд є, але картинка під ним не змінюється при скролі →
пройтись **вгору по предках** до першого з тригерів вище. Дефект майже ніколи не в самому
елементі — саме тому його не ловлять оглядом його ж стилів.

*Прецедент у родині: `Lens_glass_FINDINGS.md` §5-B ловив цей самий капкан
(«`opacity<1` на `.isl` обрізало б скло»), але записав його як окремий випадок колапсу
на скролі, а не як загальний закон. Один симптом, двічі знайдений із нуля — тепер закон.*
...
## A85. `localStorage` у PWA: що розділяє сховища і три механізми втрати ✅

**Тригер.** Продукт тримає стан користувача в `localStorage`; або постало питання
«чому дані зникли», «де безпечно тестувати польові дані», «чи безпечний бамп білда».

### Механізм (12.10 — окремо від спостереження, бо переживе зміну платформи)

**1 · Сховище прив'язане до ORIGIN = `scheme + host + port`.**
ШЛЯХ НЕ РОЗДІЛЯЄ: `/a/index.html` і `/b/index.html` на одному хості ділять одне
сховище. Розділяють: `http` vs `https`, `www` vs без `www`, інший піддомен, інший порт.
**Query-рядок не розділяє нічого** — `location.replace(pathname+'?v='+build)` з A79
(cache-bust самооновлення) для даних безпечний за побудовою.

**2 · Окремо від origin стоїть КОНТЕЙНЕР.**
На iOS застосунок, доданий на робочий стіл, має власне сховище, ізольоване від
вкладки Safari **за тією ж адресою**. Видалив іконку й додав знову — контейнер
створено з нуля; дані попереднього не переносяться і не зливаються.

**3 · Наслідок для preview-збірок.** Вони мають своє сховище **не тому, що новий
шлях**, а тому що відкриваються з іншого origin (`blob:` / `file:` / окремий хост
показу). Практичний наслідок той самий, причина інша — і саме через невірно названу
причину діагноз ставиться не туди.

### Спостереження [08.2026] — чотири механізми втрати, жоден не є дефектом коду

За замовчуванням origin живе в режимі **best-effort**: персистентність не
гарантована. Звідси чотири канали втрати, і вони б'ють по РІЗНИХ оточеннях —
плутати їх означає ставити діагноз не туди.

|# | механізм | б'є по вкладці Safari | б'є по встановленій PWA |
|---|---|---|---|
|1|**ITP, 7 днів** без взаємодії|**так**|**ні** — home-screen застосунок не є частиною Safari і має власний лічильник днів|
|2|**Витіснення під тиском місця** (best-effort)|так|**так** ← найімовірніша причина, коли PWA втратила дані|
|3|**«Clear History and Website Data»**|так|**так** — очистка Safari знімає й дані home-screen застосунку|
|4|**Перевстановлення PWA**|—|так, контейнер створюється з нуля|

**Механізм 2 знімається одним викликом — і це ЄДИНИЙ канал, на який код взагалі
має вплив:**

```js
/* origin у persistent-режимі виключено з витіснення (webkit.org/blog/14403) */
if (navigator.storage && navigator.storage.persist) {
  navigator.storage.persisted().then(ok =>
    ok ? true : navigator.storage.persist());
}
```

Дозвіл видається **без промпту** — за фактом встановлення на робочий стіл і
регулярної взаємодії. Викликати треба **при кожному поверненні у видимість**, не
один раз на старті: режим документовано скидається між сесіями. Прапорець
результату тримати **в пам'яті, не в `localStorage`** — писати діагностику в те
саме сховище, надійність якого перевіряєш, безглуздо.

**`navigator.storage.estimate()` (iOS 17+)** віддає реальні `usage`/`quota` origin:
до iOS 17 стартовий ліміт був 1 ГБ, тепер квота рахується від обсягу диска. Для
`localStorage` власний ліміт ~5 МБ лишається чинним окремо.

*Перевірити при наступному мажорному iOS.*

### Правило, що з цього випливає

**Для продукту, який тримає дані лише в `localStorage`, резервна копія — частина
функціоналу, а не зручність.** Жоден із трьох механізмів не питає дозволу, не лишає
сліду і не має undo: стерте значення не відрізняється від того, якого не було.
Канал резерву на iOS — `navigator.share({files:[...]})` → «Зберегти у Файли» →
iCloud Drive, і зворотно `<input type="file">`. Прямого JS-API до iCloud немає;
автосинхронізація вимагає бекенда й акаунта, тобто іншого класу продукту.

**Польові дані збирати ТІЛЬКИ на сталій адресі** (встановлена PWA або `index.html`
хоста), ніколи в preview-збірці.

### Анти-приклад (StockCheck, 15.08.2026)

Діагноз «дані зникли» ставився за формулюванням «сховище прив'язане до адреси
документа, новий шлях = власне сховище». Формулювання називає не той механізм:
шлях не розділяє нічого, розділяють origin і контейнер. Далі помилка повторилась
на другому колі: першим підозрюваним названо ITP, який по встановленій PWA не
б'є взагалі. Обидва рази діагноз ішов від механізму, який у цьому оточенні
неактивний, тоді як активний (витіснення best-effort) не перевірявся — і саме він
знімається п'ятьма рядками коду, яких у білді не було.

### Детектор (К2)

Рядок «Пам'ять додатка» мусить **розрізняти два стани**: `0.0 КБ` = origin порожній
(механізми 1–3) vs ненульова вага без візитів = origin живий, зник саме ключ даних.
Якщо обидва стани показуються однаково — детектора немає, і причина втрати
невстановлювана в полі. Машинна перевірка: `StockCheck_b32_6_s20_smoke.js`, блок N2,
твердження «два стани РІЗНІ на вигляд».
