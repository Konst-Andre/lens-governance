...
## A89. Граматика знаків рядка керування — однакові чіпи з різними дієсловами ламають передбачуваність ✅

**Симптом.** У рядку під шапкою стоять 3–4 візуально однакові чіпи. Один відкриває
поповер, другий перемикає стан на місці, третій щось фільтрує. Оператор тапає навмання
і щоразу дізнається, що буде, **тільки після тапу**.

**Хибний лік — «їх забагато, приберемо один».** Зменшення кількості семантику не лікує,
воно її **ховає**: два контроли з різними дієсловами так само непередбачувані, як чотири.
Лікується не кількість, а **відсутність граматики**.

### Граматика знаків 🔒

| знак | обіцянка контрола |
|---|---|
| **`⌄`** | відкриє **поверхню** поверх екрана (поповер або шіт — поріг у A90) |
| **`↓ ↑`** | перемкне стан **тут же**, без поверхні |
| **`✕`** | несе **активний вибір**, який можна зняти |
| **зміна матеріалу** *(і тільки вона)* | «фільтр **активний**» |

**Другий обов'язковий елемент: вибране мусить бути видно на самому контролі.**
`Локація ⌄` → після вибору стає `Полтава ✕`. Інакше фільтр невидимий, а список **бреше**:
показує підмножину, виглядаючи як повний.

**Стрілка сорту — ОДИН елемент, що повертається** (`transform:rotate(180deg)`),
а не підміна гліфа `↓`→`↑`. Напрям читається **рухом**; підміна дає стрибок, з якого
неможливо дізнатись, що саме змінилось.

### A89-б · Активний фільтр — МАТЕРІАЛОМ, не акцентом 🔒

Заливка активного контрола акцентним кольором конкурує з уже наявними заливками екрана
(активний таб нижнього острівця, пігулка шапки). **Три однакові кольорові плями сперечаються
за увагу** — і жодна не виграє.

> **Пасив = обведення. Актив = западина (`well`)** — той самий матеріал, що в полі пошуку.
> Обведений контрол **лежить на аркуші**; вибраний — **втиснутий у нього**.

Це словник, яким тема вже говорить (A51 tactile well, A65 дві мови глибини), а не новий
сигнал, який доводиться вчити.

- **Акцент — ні.** Акцент має рівно одну роботу на екрані. Якщо він рахує потреби,
  він **не може заразом** означати «фільтр увімкнено»: два значення одного кольору =
  жодного значення.
- **Градієнт — ні** (device-вирок Konst).

**Ставка, не лок:** `fill ‖ well ‖ tint` судяться на девайсі. Але **акцент із трійки виключений
до порівняння**, за аргументом вище — це рішення, а не варіант.

**Зв'язок:** A90 (поріг поверхні) · A51 (втоплене поле) · A65 (well vs elevation) ·
A64 (баланс контрол-ряду). *EquipLens v6/v7, 21.08.2026.*

-----

## A90. Поріг поверхні: поповер ↔ нижній шіт — один примітив на дві роботи ✅
...
**Поповер і шіт — НЕ два компоненти.** Це один примітив на дві роботи: та сама розмітка,
той самий скрим, той самий обробник закриття, різна геометрія прив'язки. У EquipLens v6
один `.el-pop` обслуговує і фільтр локації, і дропдаун теми. Заводити два — це
подвоїти площу дефектів заради різниці в `position`.

### Позиція шіта 🔒

**Шіт іде від НИЖНЬОГО РЕБРА ЕКРАНА і накриває нижній острівець**, як нативний.

Шіт, що зупиняється на верхній кромці плаваючого острівця, читається як **«застряг»** —
око бачить не «поважає хром», а «не доїхав». Спокуса зупинити його там велика саме тому,
що острівець виглядає як фізична перешкода; насправді він шар **під** шітом, а не стіна.

*Device-вирок Konst, 21.08.2026, EquipLens v7.*

**Зв'язок:** A89 (знак `⌄` обіцяє саме цю поверхню) · A6 (theme dropdown) ·
A53 (кастом тап-селект) · Cookbook том 2 (механіка bottom sheet, свайп, VisualViewport).

-----

## A91. Одна альфа на об'єкт — `alpha × opacity` робить паспорт брехливим ✅

Прозорість живе **в одному місці** — в alpha кольору. Коли на тому самому елементі
стоять **і** alpha, **і** `opacity`, побачене розходиться із записаним рівно в другий
множник — а гейт мовчить, бо обидва важелі законні й кожен окремо правильний.
Це найтихіший клас брехні паспорта: ламається не код, а **довіра до числа**.

**Прецедент.** Роздільник шапки EquipLens: `.18 × .5 = .09`. У паспорті стенда
стояло `.18`, око судило `.09`. Device-раунд виносив вирок числу, якого в кадрі не було.

**Дія 🔒**

| носій | за що відповідає |
|---|---|
| **alpha кольору** | прозорість **об'єкта** — єдиний важіль, який крутить оператор |
| **`opacity`** | прозорість **групи/шару** — поява, зникнення, весь блок разом |

Разом на одному елементі — лише з явним рядком у паспорті: *«ефективна альфа = X»*.

**Детектор (К2).** Греп стенда: елемент, що має і `rgba(...)`/`--*-a`, і власний
`opacity`, — кандидат на подвійний множник. Знайшов → або злити в один важіль,
або оголосити добуток у паспорті.

**Зв'язок:** `Lens_glass_FINDINGS` Ф-3 (скло: прозорість = alpha фону, **ніколи** не
`opacity` — інакше вмирає `backdrop-filter`) · `A45` · `A88`.
*Джерело: EquipLens S8-Г.*

-----

## A92. Підпорядкована мітка ведеться від кегля назви, висота задається явно ✅

**Формула 🔒:** `висота мітки = кегль назви + 6px`, вилка **±2**. При `nSize 14` → **20px**.

Мітка (бейдж, лічильник, статус-пігулка), що стоїть при назві, — не самостійний
об'єкт: вона **підпорядкована**, і її розмір мусить бути функцією господаря.
Прибита константа розсинхронізується з першою ж зміною кегля.

**Висота задається явно, з центруванням вмісту** — не витікає з `padding`.
Причина механічна: `padding` рахується від **лінійної висоти тексту всередині мітки**,
а вона своя й міняється від шрифта, кегля мітки та `line-height`. Тому мітка, зібрана
на `padding`, повзе не тоді, коли рухається назва, а тоді, коли рухається щось третє —
і зв'язок «назва → мітка» перестає триматись.

**Зв'язок:** `A93` (радіус тієї ж мітки) · `A64` (баланс контрол-ряду) · `A89` (граматика знаків).
*Джерело: EquipLens S8-Г.*

-----

## A93. Радіус — вибір мови, не смаку ✅

**Капсульний радіус — мова контролів.** Той самий радіус усередині контенту читається
як обіцянка: *«це можна натиснути»*. Око вчить словник на перших екранах і далі судить
за ним, а не за наміром автора.

**Правило 🔒**

| об'єкт | радіус |
|---|---|
| контрол (чіп, кнопка, сегмент, фільтр) | **капсула** |
| мітка / бейдж **усередині контенту** | **0–6** + волосяний кант |

**Перевірка перед вибором.** Якщо над списком уже стоять капсульні чіпи — капсула
**всередині** рядка створює хибну афордансію. Не тому, що вона негарна, а тому, що
в цьому екрані капсула вже **зайнята значенням** «мене тиснуть».

**Анти-приклад:** мітка стану в рядку списку, зроблена капсулою «щоб було охайно», —
кожен другий оператор пробує її натиснути.

**Зв'язок:** `A92` (та сама мітка, висота) · `A89-б` (актив = матеріалом, не акцентом) ·
`A84` (декоративний елемент, що обіцяє семантику).
*Джерело: EquipLens S8-Г.*

-----

## A94. Кнопка, що виглядає як стан і поводиться як дія 🔴

**Ряд контролів = один регістр.** Усі елементи одного ряду мусять відповідати на одне
питання: або «**що я зараз бачу**» (стан), або «**що зі мною зробити**» (дія).
Змішаний ряд дає найдорожчий сорт скарги — *«наче не працює»*: контрол працює
бездоганно, але не так, як пообіцяв власний вигляд.

**Прецедент.** `Усі 12` поруч із `Локація ⌄` і `Потреби ↓`. Сусіди — стани (показують,
що обрано). Око читає `Усі 12` як третій стан («я дивлюсь усі»), а реалізовано скидання
фільтрів. Натиск на «стан, який і так активний» → візуально нічого → «зламано».

**Дія 🔒.** Обрати один бік і довести до кінця:

| трактування | поведінка |
|---|---|
| **стан** | підсвічується, коли фільтра немає; натиск нічого не змінює |
| **дія** | **гасне/зникає**, коли скидати нічого; активна лише за живого фільтра |

Третього не дано. «Хай буде і те, і те» = повернення до дефекту.

**Детектор (К2).** Назвати **дієслово** кожного елемента ряду вголос. Якщо в одному
ряду звучать і «показую», і «зроби» — регістр змішаний, і це видно до device-раунду.

**Зв'язок:** `A89` (граматика знаків рядка керування — той самий ряд, інша вісь) ·
`A89-б` · `A64`.
*Джерело: EquipLens S10 2-Ж.*

-----

## A76. Метрична легенда, що не рве рядок ✅
