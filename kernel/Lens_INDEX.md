...
| `EquipLens_headbench_v17_26.html` | головний стенд EquipLens: 7 табів / **20 груп** (`<details>`, 7 відкритих) · **212 важелів + 34 пікери** · паритет `S=247`. Вироки запечені в `LOCK_SEED` — 214 device-вироків, стенд відкривається на ЛОКу незалежно від сховища. *(Заміряно скриптом 30.08.2026; самері S31 заявляє 35 пікерів — розбіжність на 1 не з'ясована.)* | не витіснений новим білдом стенда |
| `smoke_v17_26.js` | матриця головного стенда — **489** тверджень; `--inject` ловить **61** підкинутий дефект (заміряно 30.08.2026); ганяти з обома режимами | стенд живий |
