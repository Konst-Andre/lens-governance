...
| `Lens_patch_check_protocol.md` *(`kernel/wsd/`)* | **як перевіряти патч**: детектор в обидва боки `1.10` `1.13` `1.15` `1.16` · до видачі кластер 3 · діагноз `4.1`–`4.3` · після кластер 10. Розпил wsd С5 (G-M 18.09.2026). **Читається:** точково за номером — маршрут з таблиці на початку `wsd` |
...
**Стеля 8 192 B (8 KiB)** — число канонічне **тут**, решта файлів посилається і не переказує (`12.20`). Детектор — `kernel/check_cherga.py`, константа `CEIL`; звірено 18.09.2026 (G-H, вирок Konst). Проза «8 KB» читалась і як 8 000, і як 8 192 — саме це закривало `IDX-10`. **Запас** рахується до порогу Ф-10 — 90% стелі, **7 372 B**, не до 8 192: вище порогу черга вже сигналить «прополка замість розпилу» (Ф-24; тексти — самері GH §2 Ф-10 · GO §2 Ф-24).
...
⚠ **Стан номерів гейтів** (звірено грепом `Lens_validate.py` 18.09.2026, G-I).
Написані: `G1`–`G13`, `G16`, `G19` (спрацювання правил, К4-1), `G21` (номер правила —
один дім, `IDX-7`, G-J), `G22`, `G23`, `G24` (дубль імені, G-V). Файли правил — константа `RULE_FILES` у `Lens_validate.py`;
...
Перший вільний номер — **`G25`** (звірено регексом `G\d+` по `Lens_validate.py`, G-V 24.09.2026; `G22` — стартове в code block, G-Q · `G23` — тригер читання, К6-1 · `G24` — дубль імені, детектор Ф1, G-V). Колізію `Г-10`/`G12` розведено G-C (`Г-12'` закрито).

### Черга ядра — `kernel/Lens_governance_CHERGA.md`

Заведена 30.08.2026, **переїхала з `products/` у `kernel/` 31.08.2026**: тека визначає
роль, а цей носій тримає борги **ядра** (wsd · Cookbook · маніфести · гейти · індекси),
не продукту. Раніше такі пункти жили в `EquipLens_CHERGA.md` і витісняли з неї
продуктове. Правила й стеля — ті самі, що в черг продуктів. Читається **цілком** на
старті governance-сесії; на старті продуктової — за потреби.
...
| **Lens** *(governance)* | `Lens_governance_session_summary_GY_GIT_APP_2B.md` · `Lens_governance_session_summary_GX_START_APP.md` · супутник `Lens_inventory_GT_v1.md` · тимчасовий журнал переїзду `Lens_MIGRATION_GW_ledger.md` (живе доки перенос Ф3 не завершено, потім видаляється) — **лежать у репо, тека `sessions/Lens_gov/`, не в Project** (Ф2 для governance, `Lens_REPO_LAYOUT.md` §2, G-U 24.09.2026; читати з clone/raw) *(GW — G-Y; GV — G-Y; GU — G-W; GT — G-V; GR · GQ витіснено G-U, файли не збережено — зміст у комітах `39681fd`…`fcfc3ec`; F · G — сироти G-C3; GP — G-S; GO — G-R; GN — G-Q; GM — G-P; GL — G-O; GK — G-N; GJ — G-M; GI — G-L; GH — G-K; GG — G-J; GF — G-H; GA…GE — 17.09.2026 → `archive/summaries/Lens_gov/`)* |
...
1. **існування** — кожен файл, названий тут, є в Project або в репо; немає → ✗ (це
   зловило б обидва фантоми вище);
...
1. **Новий або правлений рядок канону — шляхом від кореня репо** (`Репо:шлях` — для чужого);
   чинно з G-V 24.09.2026 (хід детектора Ф1: `G8` резолвить шлях · `G7` без хибного ✗ · `G24`).
   Старі посилання іменем дійсні, доки ім'я унікальне — масового перепису немає: `Lens_REPO_LAYOUT.md` §1.
2. **Ім'я мусить бути унікальним по всьому дереву.** Два однойменні файли в різних
   підтеках зіллються в Project у невизначений один. Детектор — **`G24`** (G-V): дубль у живому
   дереві → ✗, лише в `archive/` → ⓘ. До G-V це був безномерний скан перед `G1` (з 01.08.2026),
   що валив ✗ і на `archive/`, якого не правлять; G-U його не побачила й назвала «детектора немає».
