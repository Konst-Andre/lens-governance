...
**Area (область SR) НЕ приходить з Excel — обмеження, а не баг.** Тунель Area-чіпів
у `#sh-sr` (B60/B61) розкладає представників по областях через **хардкод-літерали в HTML**:
`SR_AREA` (мапа «ПІБ → область») і `AREA_ORDER` (порядок чіпів). У `_QR_Data` такого поля немає,
`BuildQRJSON` його не емітить, у схемі §4 його теж немає.

*Наслідок для регенерації:* мапа запікається в `QR_Lens_template_v2.html` разом з рештою HTML.
Новий представник, якого немає в `SR_AREA`, потрапить у фолбек-чіп «Інші» — і полагодити це
можна буде ЛИШЕ ручною правкою шаблону. Порожніх чіпів при цьому не буває структурно
(`areaBuckets` будує кошики з фактичного складу DATA), тож зламаного вигляду не буде — буде
тихо неправильна розкладка.

*Рішення відкладено свідомо* (Konst, 17.08.2026): поки склад команди сталий, ручна правка
дешевша за зміну схеми. Коли представників побільшає — правильний хід не в HTML, а в Excel:
нова колонка області в джерелі → поле в `SerializeSR` → фронтенд читає `d.area` замість мапи.
Це вимагає правки VBA (див. §5, гілка «нове поле в DATA») і бампу контракту на v2.

*Детектор:* представник, який точно має свою область, показується під чіпом «Інші» →
дивитись сюди, не в JS розкладки.
