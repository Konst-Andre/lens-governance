...
- **перенесено** ✓ CANON (device✓ в продукті b14_2, 20.07.2026): **НЕ ховаємо ринг** — картка тримає **ринг + fill + гepo-число** (як ready), лише dim. `✓✓` **у розриві дуги** (замість `%`, P1) + tiny-caption «перенесено» під рингом. *rationale (Konst):* стара голі-`✓✓`-заміна ховала дизайн, який дорого зробили; ще й суперечність — усі неактивні картки мали неактивну дугу, тож «схована власним успіхом» (§5.2-deferred hole). Показ рингу на перенесеній підсилює «переніс усі показники» + картка читається преміальніше.
  ...
  - Це **статичне** закриття hole «дуга схована»; **celebration-на-копі** = закрито в b27 як **handoff-sweep** (Cookbook A82), не дугою. hole #1 → WONTFIX (§9).
...
1. ~~**Motion-стенд (hole #1)**~~ — **WONTFIX 30.07.2026** (b27 §3). Спростовано реальним маршрутом: на Home після переносу представник не повертається, тож reorder не обслуговує ні гедонічну функцію (глядача немає), ні інформаційну (картка вже статична). Нагороду моменту закрито інакше — handoff-sweep на 📋 (Cookbook **A82**). Повний розбір: `StockCheck_MASTER_LOCK.md` §11. **Не повертати в чергу.**
...
**? OPEN-design (harness-first, device-арбітр):** ~~Home-картка (3 стани)~~ ✅ LOCKED §5.2/§6.4 · ~~пікер-рядок~~ ✅ §5.3 · ~~§5.1 UI верхівка~~ ✅ DESIGN-LOCKED §5.1 · ~~status-фільтр~~ ✅ §5.4 · **лишається:** colhead-«дім» (§5.1 HIGH-3) · date-picker A58 від/до. ~~hole #1 motion~~ → **WONTFIX** (вище).
