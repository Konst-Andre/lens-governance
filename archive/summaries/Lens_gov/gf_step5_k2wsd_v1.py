#!/usr/bin/env python3
# живе доки: G-F5 запушено й прочитано назад — тоді archive/summaries/Lens_gov/
# G-F5: К2-1 wsd група 2 — детектори/тригери/K2:n/a за GF5_k2_wsd_texts_v1.md §2 + шапка 2.37.
# Тексти — лише з файла текстів (не дублюються тут). Прохід 1 — перевірки в пам'яті; прохід 2 — запис (Ф-4). Повтор → no-op.
import sys, os, re, hashlib
ROOT = sys.argv[1] if len(sys.argv) > 1 else '.'
TEXTS = sys.argv[2] if len(sys.argv) > 2 else '/mnt/user-data/outputs/GF5_k2_wsd_texts_v1.md'
...
RULES = ['2.2', '2.3', '2.4', '2.10', '3.3', '3.4', '3.6', '3.7', '3.10']
TRIG = ['3.3', '3.4', '3.7', '3.10']
...
for k, v in blocks.items():
    ...
    if k.endswith('det') and k != '3.3 det' and not v.startswith('**Детектор (К2).**'): die(f'{k}: не починається з **Детектор (К2).**')
    if k.endswith('trig') and not v.startswith('**Тригер.**'): die(f'{k}: не починається з **Тригер.**')
if not re.fullmatch(r'<!-- K2:n/a\s*—\s*\S.*-->', blocks['3.3 det']): die('3.3: маркер K2:n/a не за формою (латинська K)')
...
for r in RULES:
    ...
    if trig:
        ...
        rest = s[len(head) + 2:]
        if rest.startswith('> '):                      # блокквота «Формалізація» одразу під заголовком
            q = rest.split('\n\n', 1)
...
VER_ANCH = '> **Версія 2.36** (17.09.2026'
ver = blocks['wsd ver']
if new.count(ver) == 1: done.append('ver')
elif new.count(VER_ANCH) == 1 and new.count('**Версія 2.37**') == 0:
    new = new.replace(VER_ANCH, ver + '\n' + VER_ANCH); todo.append('ver')
else: die(f'шапка: якір 2.36={new.count(VER_ANCH)} 2.37={new.count("**Версія 2.37**")}')
