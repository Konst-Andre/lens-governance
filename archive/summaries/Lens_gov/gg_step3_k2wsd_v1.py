#!/usr/bin/env python3
# живе доки: G-G3 запушено й прочитано назад — тоді archive/summaries/Lens_gov/
# G-G3: К2-1 wsd група 3 — детектори/тригери/K2:n/a за GG2_k2_wsd_texts_v1.md §2 + шапка 2.38.
# Тексти — лише з файла текстів (не дублюються тут). Прохід 1 — перевірки в пам'яті; прохід 2 — запис (Ф-4). Повтор → no-op.
import sys, os, re, hashlib
ROOT  = sys.argv[1] if len(sys.argv) > 1 else '.'
TEXTS = sys.argv[2] if len(sys.argv) > 2 else '/mnt/user-data/outputs/GG2_k2_wsd_texts_v1.md'
WSD   = os.path.join(ROOT, 'kernel', 'wsd', 'Work_Standard.md')
def die(m): print('STOP:', m); sys.exit(1)

RULES = ['4.1', '4.2', '10.1', '10.2', '10.3', '10.4', '10.5', '10.6', '10.7', '10.8']
TRIG  = ['4.1', '4.2', '10.5']
NA    = ['4.2', '10.1', '10.2', '10.5']
...
for r in RULES:
    ...
    if trig:
        ...
        rest = s[len(head) + 2:]
        if rest.startswith('> '): die(f'{r}: очікувався звичайний текст, а не блокквота (§1 файла текстів)')
        s = head + '\n\n' + trig + '\n\n' + rest
    if '> *Походження' in s: die(f'{r}: є «Походження» — гілка не передбачена цим скриптом')
    if s.rstrip('\n').endswith('-----'): die(f'{r}: сегмент закінчується роздільником — детектор пішов би поза правило')
    tail = s[len(s.rstrip('\n')):]
    s = s.rstrip('\n') + '\n\n' + det + '\n' + tail
    if s.count('**Детектор (К2).**') + s.count('K2:n/a') < 1: die(f'{r}: після збірки немає детектора')
    new = new.replace(seg, s); todo.append(r)

VER_ANCH = '> **Версія 2.37** (17.09.2026'
ver = blocks['wsd ver']
if new.count(ver) == 1: done.append('ver')
elif new.count(VER_ANCH) == 1 and new.count('**Версія 2.38**') == 0:
    new = new.replace(VER_ANCH, ver + '\n' + VER_ANCH); todo.append('ver')
else: die(f'шапка: якір 2.37={new.count(VER_ANCH)} 2.38={new.count("**Версія 2.38**")}')
